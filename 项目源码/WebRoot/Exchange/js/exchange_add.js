$(function () {
	...
	$("#exchange_employeeObj_employeeNo").combobox({
        ...
        required : true,
        onLoadSuccess: function () { //数据加载完毕事件
            var data = $("#exchange_employeeObj_employeeNo").combobox("getData"); 
            if (data.length > 0) {
                $("#exchange_employeeObj_employeeNo").combobox("select", data[0].employeeNo);
            }
        }
	});
	//单击添加按钮
